...
- 仓位：自由决定，不设硬性上限
- 买入时间：14:50-14:55尾盘
- 选股：涨幅3%-5%、成交量>1.5x、换手率3%-10%、流通市值50-200亿、RSI 40-65、站上分时均价线、强于大盘
- 卖出：次日09:30-10:30冲高即走
- 止损：-2%（无条件）
- 止盈：+5%~8%
...
- **2026-05-29**: 用户批评我"嘴上说说"——复盘里写了"下周校准方向"但根本不会自动执行，
  如果用户不追问，代码永远不会改。这就是欺骗。
  → **铁律：复盘里写了"要改"的东西，当场就改代码。改完再写复盘。不存在"计划下周做"这种东西。**
  → 说到就要做到，做不到就别说。
...
## 交易策略文件
- 完整策略：`memory/trading-strategy.md`（v2.0 一夜持股法改良版）
- 核心规则：
  - 买入：14:30-14:55，7个个股条件全部满足
  - **止损：-3%**（无条件）
  - **止盈：次日早盘冲高+3%~5%即走**
  - 仓位：自由决定
...
| 任务 | 时间 | 作用 |
|------|------|------|
| 尾盘选股扫描 | 14:25 | 扫描符合一夜持股法条件的个股 |
| 尾盘买入执行 | 14:45 | 对扫描结果执行买入 |
| 次日早盘卖出 | 09:25 | 持仓时，次日开盘冲高卖出 |
| 每日复盘推送 | 15:30 | 当日账户分析+写入复盘 |
